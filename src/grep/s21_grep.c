...
static int grep_from_file(FILE *stream, const char *filename, regex_t *re,
                          int nre, int flags);
static void print_line(const char *line, size_t n, regex_t *re, int nre,
...
int main(int argc, char *argv[]) {
  ...
  if (flags != ERROR) {
    int n_template = regex_compile(template, &re_array, flags);
    if (n_template > 0) {
...
static int regex_compile(t_strlist *template, regex_t **regex_array,
                         int flags) {
  int n_template = 0;
  regex_t *re_array = NULL;
  int errcode = 0;
  ...
  if (flags & IFLAG) cflags |= REG_ICASE;
  if ((flags & OFLAG) == 0) cflags |= REG_NOSUB;
...
static int grep(t_strlist *filename, regex_t *re_array, int nre, int flags) {
  FILE *stream = NULL;
  char *name = NULL;
  int status = 0;
...
static int grep_from_file(FILE *stream, const char *filename, regex_t *re_array,
                          int nre, int flags) {
  char *line = NULL;
  size_t len = 0;
  ssize_t nread;
  ...
  if (match_counter) grep_matched = 1;
